<?php
	...
	$fx = $data->pickupaddress->lat;
	$fy = $data->pickupaddress->long;
	$tx = $data->dropoffaddress->lat;
	$ty = $data->dropoffaddress->long;
	...
	//this area for modifying type of vehicles & their prices
  //dynamically push items onto array with key,value and then return that array to be encoded
	  //$j[key]=value

function dbcar($price){
	$DBH = new PDO("mysql:host=localhost;dbname=limo", 'root', 'apple360');
	$DBH->setAttribute( PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION );
	$STH = $DBH->query("SELECT * FROM `cars`");
	$STH->setFetchMode(PDO::FETCH_ASSOC);
	while($row = $STH->fetch()){
		$j[$row["type"]]=($row["price"]*$price);
	}
	return $j;
}
